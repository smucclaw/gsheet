...
docker run -v /etc/letsencrypt:/etc/letsencrypt natural4-server:prod
#+end_example

Note that we tag each docker image with their respective ~dev~ or ~prod~ builds. This helps us differentiate which one to run. The production image is intended to be ran on the production server. It will need the letsencrypt certificates.
