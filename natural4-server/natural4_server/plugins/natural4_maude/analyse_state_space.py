...
maude_main_file: pathlib.Path = pathlib.Path(__file__).parent / "main.maude"
maude_main_mod = init_maude_n_load_main_file(maude_main_file)


@curry
def gen_state_space_and_find_race_cond(
    output_path: str | os.PathLike, config: maude.Term, natural4_rules: str
):
    """
...
async def get_maude_tasks(
    natural4_file: str | os.PathLike, output_path: str | os.PathLike
    ...
    if natural4_rules:
        ...
        config: maude.Term | None = await asyncio.to_thread(
            natural4_rules_to_config, maude_main_mod, natural4_rules
        )
